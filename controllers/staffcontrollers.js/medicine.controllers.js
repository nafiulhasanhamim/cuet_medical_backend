...
//get particular medicine info like medicine_name, image and mg
const getParticularMedicineInfo = async (req, res) => {
  const { medicine_id } = req.params;
  try {
    const medicineInfo = await Medicines.find({ medicine_id });
    if (medicineInfo) {
      res.status(200).json({
        success: true,
        message:
          "returns particular the medicines information like medicine_name, image and mg",
        medicineInfo,
      });
    } else {
      res.status(200).json({
...
const addMedicineStockInfo = async (req, res) => {
  // const { role } = req.role;
  let { medicine_id, _id, price, piece } = req.body;
  price = Number(price);
  piece = Number(piece);
...
const editMedicineStockInfo = async (req, res) => {
  ...
  try {
    // if (role === "staff" || role === "admin") {
      ...
      medicineInfo.availableStock =
        medicineInfo.availableStock - prevStock + piece;
